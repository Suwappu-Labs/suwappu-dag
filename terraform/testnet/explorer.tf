...
#   * Alias `explorer.testnet.suwappu.globalsettlement.com`.
#   * Reuses this stack's wildcard ACM from acm.tf.

resource "aws_s3_bucket" "explorer" {
  provider      = aws.us_east_1
  bucket        = "suwappu-dag-testnet-explorer"
  force_destroy = false

  tags = { Name = "suwappu-testnet-explorer" }
}
...
resource "aws_cloudfront_origin_access_control" "explorer" {
  provider                          = aws.us_east_1
  name                              = "suwappu-testnet-explorer-oac"
...
resource "aws_cloudfront_distribution" "explorer" {
  ...
  comment             = "suwappu-testnet explorer SPA"
