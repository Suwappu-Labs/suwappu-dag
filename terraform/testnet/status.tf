# Testnet status page — static-site deployment.
#
# Fork of terraform/devnet/status.tf with bucket + DNS rewired
# for testnet. Same shape as testnet/explorer.tf minus the SPA
# fallback rules (status page is a single index.html).
...
resource "aws_route53_record" "status" {
  ...
  name     = "status.${var.testnet_subdomain}.${var.apex_domain}"
  type     = "A"
...
output "status_page" {
  ...
  value = {
    url             = "https://status.${var.testnet_subdomain}.${var.apex_domain}"
    bucket          = aws_s3_bucket.status.id
