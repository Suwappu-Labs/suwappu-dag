...
#
# Reuses the perf stack's `/suwappu-perf/suwappu-db-deploy-key` SSM parameter
# (same suwappu-db repo, same deploy key) — kept under one path to avoid
# rotating two secrets in lockstep.

resource "aws_iam_role" "codebuild" {
  provider = aws.us_east_1
  name     = "suwappu-testnet-codebuild"
...
resource "aws_codebuild_project" "build" {
  provider     = aws.us_east_1
  name         = "suwappu-testnet-build"
  description  = "Native-compile suwappu-node binaries to aarch64-unknown-linux-gnu for the testnet seed cluster"
  ...
  logs_config {
    cloudwatch_logs {
      group_name  = "/aws/codebuild/suwappu-testnet-build"
      stream_name = "main"
    }
  }
}
