...
## AWS-name prefix: `suwappu-dev-*`

This stack passes `name_prefix = "suwappu-dev-"` to the validator module
in [`./modules/validator/`](./modules/validator/). The prefix is
deliberately **not** the historical `suwappu-devnet-` — that namespace
...
Name fields under this stack therefore read `suwappu-dev-<region>`. The
next clean window for the testnet will flip testnet to
`suwappu-testnet-*` and free up `suwappu-devnet-*` for the devnet; until
