...
TypeScript client SDK for the [`suwappu-dag`](https://github.com/Suwappu-Labs/suwappu-dag) JSON-RPC query API.
...
```sh
npm install @suwappu/client
# or
pnpm add @suwappu/client
# or
bun add @suwappu/client
```
...
if (stake === null) {
  ...
  console.log(`stake: ${stake.stake_suwappu} SUWAPPU`);
}
```
...
| `getValidatorRegistry()` | `ValidatorMemberView[]` | `stake_suwappu` is a decimal **string** (u128 doesn't fit in `number`) |
...
const stake = await client.getStake(0);
if (stake) {
  const asBigInt = BigInt(stake.stake_suwappu);
...
All thrown errors extend `SuwappuClientError`:
...
try {
  ...
  } else if (err instanceof SuwappuClientError) {
...
No CI step in `.github/workflows/ci.yml` runs this — suwappu-dag's CI is Rust-only. Publish/test wiring lands in a follow-up.
...
The Rust SDK (`suwappu-client`) follows the same policy and pins exactly the same minor-version cadence; method names map between camelCase (TS) and snake_case (Rust), but the shape and stability of the surface are identical.
