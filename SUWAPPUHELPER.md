...
This file holds the project context for the repo. It is imported by the root
`CLAUDE.md` stub (`@./SUWAPPUHELPER.md`), so Claude Code auto-loads it at the start
of every session. It is the entry point for orienting Claude Code on
conventions, current sprint state, load-bearing invariants, and how to
collaborate.
...
[`suwappu/suwappu-db`](https://github.com/suwappu/suwappu-db)
...
- This `SUWAPPUHELPER.md` (sprint backlog section below)
...
1. Read this `SUWAPPUHELPER.md` (already loaded via the root `CLAUDE.md` import).
...
Treat changes to `SUWAPPUHELPER.md` like any other PR — review and merge.
