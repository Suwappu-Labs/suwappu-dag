...
- `suwappu-execution` calls `suwappudb_bridge::Bridge::submit` for intent
...
1. **Lane separation** — `suwappudb-lane` never imports into `suwappudb-state`.
...
- **Engineering:** `Cargo.toml` workspace deps (`suwappudb-bridge`,
  `suwappudb-state`, `suwappudb-lane`); `crates/suwappu-execution/src/lib.rs`
  exercises the integration.
- **Spec:** suwappu-db paper-additions
  [`dag-l1-section-7-4.md`](https://github.com/Suwappu-Labs/suwappu-db/blob/main/docs/paper-additions/dag-l1-section-7-4.md)
  is the canonical writeup of the integration surface.
- **Substrate docs:** the full suwappu-db doc tree lives in
  [`suwappu-db/docs/`](https://github.com/Suwappu-Labs/suwappu-db/tree/main/docs)
