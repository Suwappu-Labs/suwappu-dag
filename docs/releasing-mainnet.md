...
cosign verify-blob \
    ...
    --certificate-identity-regexp "^https://github.com/suwappu/suwappu-dag/" \
...
slsa-verifier verify-artifact suwappu-node-v1.0.0-aarch64-linux-musl \
    ...
    --source-uri github.com/suwappu/suwappu-dag \
    --source-tag v1.0.0
```
...
```sh
git clone https://github.com/suwappu/suwappu-dag
cd suwappu-dag
git checkout v1.0.0
...
gh release download $RELEASE --repo suwappu/suwappu-dag \
    --pattern "$BIN*"
...
git clone --depth 1 --branch $RELEASE \
    https://github.com/suwappu/suwappu-dag
cd suwappu-dag
