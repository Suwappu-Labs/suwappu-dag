...
**Status:** Ratified 2026-05-15 — Option A (multi-anchor scan in `decide_slot`) landed; paper-side amendment in [suwappu-papers#2](https://github.com/Suwappu-Labs/suwappu-papers/pulls) (pending)
**Owner:** consensus
**Date:** 2026-05-15
**Tracking:** [#35](https://github.com/Suwappu-Labs/suwappu-dag/issues/35) (closed by [#44](https://github.com/Suwappu-Labs/suwappu-dag/pull/44), test-side mitigation) → [#45](https://github.com/Suwappu-Labs/suwappu-dag/issues/45) (consensus-side fix in this IQ)
...
DAG via orphan-pull (`crates/suwappu-node/src/daemon.rs:659-686`), but by
...
Should `suwappu-consensus` close this liveness gap, and if so, how?
...
1. **Parent-set selection at propose time**
   (`crates/suwappu-node/src/daemon.rs::run_round_driver`, ~line 1544):
   ```rust
...
2. **Direct commit rule**
   (`crates/suwappu-consensus/src/commit.rs::try_direct_decide`, line 126):
   ```rust
...
3. **Indirect commit rule**
   (`crates/suwappu-consensus/src/commit.rs::try_indirect_decide`, line 149):
   ```rust
...
- **Where:** `crates/suwappu-consensus/src/commit.rs::decide_slot` +
  trigger from `crates/suwappu-node/src/daemon.rs::ingest_cert` on
...
- **Where:** `crates/suwappu-node/src/daemon.rs::run_round_driver`
...
```rust
// crates/suwappu-consensus/tests/proptest_late_arrival.rs
proptest! {
    #[test]
...
3. In `try_commit` (`crates/suwappu-node/src/daemon.rs:1125`),
...
- `crates/suwappu-node/src/daemon.rs::run_round_driver` parent-set
  selection site
- `crates/suwappu-consensus/src/commit.rs` — `decide_slot`,
