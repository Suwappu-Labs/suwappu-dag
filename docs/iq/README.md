...
| [IQ-012](./IQ-012-threshold-mldsa-checkpoint-cosignature.md) | Threshold ML-DSA for Authority checkpoint co-signature | N detached ML-DSA-65 sigs in `checkpoint.rs` (DAG-S11); no aggregate | Recommendation, pending sign-off |
| [IQ-013](./IQ-013-pq-confidential-transfers.md) | PQ auditable confidential transfers (CONF-1 / Track H) | Phase 1 shipped (SHA3 note commitment/nullifier/L2-address); Phase 2 (ML-KEM viewing keys + AEAD) + Phase 3 (STARK range/spend) unbuilt | Recommendation, pending sign-off |
| [IQ-014](./IQ-014-machine-checked-invariant-proofs.md) | Machine-checked obligations for Theorem 2 + LTP soundness (formal-verification parity vs BTX) | **Phase 1 landed** — Kani harnesses in `joint.rs` + `attestation.rs`, `scripts/verify/run_all.sh`, non-blocking CI job; see [`../audit/formal-verification.md`](../audit/formal-verification.md). Phase 2 (proof-assistant) not started | Recommendation, pending sign-off |
