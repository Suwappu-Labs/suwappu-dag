...
| [IQ-008](./IQ-008-bounded-retention-and-checkpoint-sync.md) | Bounded DAG retention (consensus-anchored GC), durable commit log, checkpoint-anchored joiner sync (`/goal` A6 + A7) | DAG-S34, implemented on `claude/kasper-research-n9i4ct`; eleven consensus-reviewer passes, no HIGH/MED remaining | Recommendation, pending consensus-team sign-off |
| [IQ-009](./IQ-009-certificate-availability.md) | Certificate availability: a certificate enters the DAG only with its block (Narwhal/Mysticeti admission rule); crashed author no longer halts the mesh (`/goal` A11) | DAG-S35, implemented on `claude/kasper-research-n9i4ct`; seven consensus-reviewer passes, no HIGH/MED remaining | Recommendation, pending consensus-team sign-off |
| [IQ-010](./IQ-010-deterministic-committee-transitions.md) | Deterministic committee transitions: committee-indexed rotation and quorum, activation and ejection as functions of the commit sequence, on-chain equivocation evidence (`/goal` A12) | DAG-S36, in progress on `claude/kasper-research-n9i4ct` | Recommendation, pending consensus-team sign-off |
