...
| [IQ-009](./IQ-009-certificate-availability.md) | Certificate availability: a certificate enters the DAG only with its block (Narwhal/Mysticeti admission rule); crashed author no longer halts the mesh (`/goal` A11) | DAG-S35, implemented on `claude/kasper-research-n9i4ct` | Recommendation, pending consensus-team sign-off |
