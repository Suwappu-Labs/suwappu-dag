...
| DAG-S36 | IQ-010 — deterministic committee transitions: committee-indexed rotation, activation and ejection as functions of the commit sequence, on-chain equivocation evidence | 🔍 Review | `proptest_committee.rs` (4) × 10k, `activation_is_a_commit_sequence_function`, `middle_ejection_keeps_the_mesh_committing` (4-node fault injection), `bogus_evidence_is_dropped`; consensus-reviewer + human sign-off gate merge |
| DAG-S35 | IQ-009 — certificate availability: a certificate enters the DAG only with its block | 🔍 Review | `availability_is_an_admission_invariant` × 10k (I-AV1 over arbitrary certificate/block/wrong-block interleavings), `cert_is_admitted_only_with_its_block`, `withholding_author_does_not_stall_the_mesh` (4-node fault injection); seven consensus-reviewer passes, no HIGH/MED remaining; human sign-off (IQ-009 Residual 9) gates merge |
