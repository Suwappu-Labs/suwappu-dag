...
- [x] A6. ~~**No persistence — the DAG store never prunes.**~~ CLOSED at the code level by DAG-S34 (IQ-008, `docs/iq/IQ-008-bounded-retention-and-checkpoint-sync.md`): the DAG and every side table are pruned to `gc_depth_rounds` (manifest, default 256) behind the leader frontier with a commit floor that is a function of the leader alone (I-GC1, `proptest_gc.rs` × 10k); with `data_dir` set a node keeps a blake3-chained commit log plus snapshots at every checkpoint boundary and resumes from disk without re-signing a round (`proptest_persistence.rs` × 10k, `restart_resumes_from_disk_without_equivocating`). GATE: consensus-reviewer verdict recorded in the PR body + human consensus sign-off before merge (IQ-008 Residual 4). Regenesis is no longer the plan.
- [x] A7. ~~**Snapshot/checkpoint sync.**~~ CLOSED at the code level by DAG-S34.4: seated authorities co-sign a checkpoint (v2, binds the committee via `registry_root`) every `checkpoint_cadence_rounds`; a joiner whose peers have pruned past its DAG verifies the checkpoint chain from the genesis Authority Ring (`verify_checkpoint_chain`, `proptest_checkpoint_chain.rs` × 10k), installs the snapshot the chain vouches for, and catches up through the same joint-quorum AND-gate as a live node (votes are relayed with backfilled certificates). Exercised end to end in `joiner_bootstraps_from_cosigned_checkpoint_snapshot`. Same merge gate as A6.
...
- [x] A11. ~~**A crashed author halts the mesh.**~~ Found by the DAG-S34 restart test: a certificate could be admitted without its block, and `try_commit` (correctly) never applies a certificate whose authentic block it lacks, so an author that crashed between broadcasting its certificate and its block froze every node's commit frontier until it returned. CLOSED at the code level by DAG-S35 (IQ-009, `docs/iq/IQ-009-certificate-availability.md`): a certificate enters the DAG only together with its block, parents are chosen only from held certificates, and the Validator-Ring vote is cast at admission — so a reference implies the referrer holds the payload (Narwhal §4.2 / Mysticeti §III). Exercised by `withholding_author_does_not_stall_the_mesh`, the first `/goal` B2 fault-injection test (`State::withhold_blocks`). Same merge gate as A6/A7.
- [x] A12. ~~**Membership changes broke the rotation and forked on timing.**~~ Found by the DAG-S35 review (IQ-009 Residual 9): the commit rule indexed authorities by `0..n` while ids are caller-chosen and never renumbered, so ejecting any non-highest id left a dead leader slot and an author nobody referenced (a permanent halt at threshold); and activation (Issue #18 deferred activation) and the DAG-S30.1 auto-eject fired on each node's own observation, so honest nodes could decide leaders with different `n` — a fork with zero Byzantine behaviour. CLOSED at the code level by DAG-S36 (IQ-010, `docs/iq/IQ-010-deterministic-committee-transitions.md`): committee-indexed rule, membership changes only at the committed epoch boundary, activation gated on a committed certificate, ejection by on-chain `EquivocationEvidence`. `State::equivocate` is the second B2 fault knob. Same merge gate as A6/A7/A11.
