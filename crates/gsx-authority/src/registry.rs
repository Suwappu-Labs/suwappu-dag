...
//!    `AUTHORITY_STAKE_THRESHOLD_GSX` (100,000 GSX).
...
//! This is the same formula as `gsx-consensus::quorum_threshold` and the
...
pub struct AuthorityMember {
    ...
    pub id: AuthorityId,
    /// Posted base-chain PoS stake in GSX. Must be at least
    /// `AUTHORITY_STAKE_THRESHOLD_GSX` at admission time.
    pub stake_gsx: u64,
    /// ML-DSA-65 public key bytes. Phase-1 stores the bytes opaquely;
    /// signature verification consumes them via
    /// `gsx_crypto::mldsa::PublicKey::from_bytes`.
...
impl AuthorityRegistry {
    ...
    pub fn admit(&mut self, member: AuthorityMember) -> Result<(), AdmissionError> {
        if member.stake_gsx < AUTHORITY_STAKE_THRESHOLD_GSX {
            return Err(AdmissionError::StakeBelowFloor {
                posted: member.stake_gsx,
                floor: AUTHORITY_STAKE_THRESHOLD_GSX,
            });
        }
...
mod tests {
    ...
    fn admit_above_floor_succeeds() {
        ...
        r.admit(member(0, AUTHORITY_STAKE_THRESHOLD_GSX)).unwrap();
        assert_eq!(r.len(), 1);
        assert!(r.contains(0));
    }
    ...
    fn admit_below_floor_fails() {
        let mut r = AuthorityRegistry::new();
        let err = r.admit(member(0, AUTHORITY_STAKE_THRESHOLD_GSX - 1));
    ...
    fn admit_duplicate_fails() {
        let mut r = AuthorityRegistry::new();
        r.admit(member(0, AUTHORITY_STAKE_THRESHOLD_GSX)).unwrap();
        let err = r.admit(member(0, AUTHORITY_STAKE_THRESHOLD_GSX));
