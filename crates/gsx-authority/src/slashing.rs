...
mod tests {
    use super::*;
    use crate::{AuthorityMember, AUTHORITY_STAKE_THRESHOLD_GSX};
    ...
    fn slash_removes_member_and_returns_full_stake() {
        ...
        assert_eq!(slash.stake_lost, AUTHORITY_STAKE_THRESHOLD_GSX);
    ...
    fn readmit_after_slash_requires_fresh_stake() {
        ...
        r.admit(member(0, AUTHORITY_STAKE_THRESHOLD_GSX)).unwrap();
        slash_authority(&mut r, 0).unwrap();
        readmit_authority(&mut r, 0, AUTHORITY_STAKE_THRESHOLD_GSX, vec![0; 32]).unwrap();
        assert!(r.contains(0));
    }
}
