...
/// `cm = SHA3-256-domain(GSX_L2_NOTE_COMMIT_V1, v_le ‖ r ‖ pk_owner)`.
/// See Track H confidential-transfer construction.
pub const GSX_L2_NOTE_COMMIT_V1: &[u8] = b"gsx-l2-note-commit-v1";

/// Domain-separation tag for L2 nullifiers
/// `nf = SHA3-256-domain(GSX_L2_NULLIFIER_V1, nk ‖ cm ‖ position_le)`.
pub const GSX_L2_NULLIFIER_V1: &[u8] = b"gsx-l2-nullifier-v1";

/// Domain-separation tag for L2 nullifier-key derivation
/// `nk = SHA3-256-domain(GSX_L2_NF_KEY_V1, sk_seed)`.
pub const GSX_L2_NF_KEY_V1: &[u8] = b"gsx-l2-nf-key-v1";

/// Domain-separation tag for L2 viewing-key seed derivation
/// `viewing_seed = SHA3-256-domain(GSX_L2_VIEWING_KEY_V1, sk_seed)`;
/// `(ek_view, dk_view) = ML-KEM-768.KeyGen(viewing_seed)`.
pub const GSX_L2_VIEWING_KEY_V1: &[u8] = b"gsx-l2-viewing-key-v1";

/// Domain-separation tag for L2 confidential-account address derivation
/// `addr = SHA3-256-domain(GSX_L2_ADDRESS_V1, pk_owner_mldsa)[..20]`.
pub const GSX_L2_ADDRESS_V1: &[u8] = b"gsx-l2-address-v1";
...
///
/// ```
/// use gsx_crypto::hash::hkdf_sha3_256;
///
...
mod tests {
    ...
    fn l2_domain_tags_are_distinct() {
        let tags: &[&[u8]] = &[
            GSX_L2_NOTE_COMMIT_V1,
            GSX_L2_NULLIFIER_V1,
            GSX_L2_NF_KEY_V1,
            GSX_L2_VIEWING_KEY_V1,
            GSX_L2_ADDRESS_V1,
        ];
    ...
    fn l2_domain_tags_separate_digests() {
        ...
        let h_commit = sha3_256_domain(GSX_L2_NOTE_COMMIT_V1, payload);
        let h_nullifier = sha3_256_domain(GSX_L2_NULLIFIER_V1, payload);
        let h_nf_key = sha3_256_domain(GSX_L2_NF_KEY_V1, payload);
        let h_view = sha3_256_domain(GSX_L2_VIEWING_KEY_V1, payload);
        let h_addr = sha3_256_domain(GSX_L2_ADDRESS_V1, payload);
