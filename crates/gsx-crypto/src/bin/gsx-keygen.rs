//! `gsx-keygen` — mint a real PQ / classical keypair and write raw bytes to disk.
//!
...
//! `gsx_crypto::mldsa::SecretKey::from_bytes` round-trips.
...
struct Args {
    ...
    /// can lift the same blake3 truncation `gsx_faucet::address_from_pubkey`
...
fn main() -> std::io::Result<()> {
    ...
    match args.algo {
        Algo::Mldsa => {
            ...
            eprintln!(
                "gsx-keygen: wrote ml-dsa-65 sk={} ({} B) pk={} ({} B)",
                args.sk.display(),
                sk.as_bytes().len(),
                args.pk.display(),
                pk.as_bytes().len(),
        ...
        Algo::Bls => {
            ...
            eprintln!(
                "gsx-keygen: wrote bls12-381 sk={} ({} B) pk={} ({} B)",
                args.sk.display(),
                sk.to_bytes().len(),
                args.pk.display(),
                pk.to_bytes().len(),
            ...
            if args.addr.is_some() {
                eprintln!("gsx-keygen: --addr is ignored for --algo bls");
            }
        }
    }
    Ok(())
