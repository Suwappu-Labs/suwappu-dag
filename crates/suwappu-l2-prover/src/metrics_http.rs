...
//! cloudwatch-agent. Mirrors `suwappu-node`'s `metrics_http.rs`.
...
pub async fn serve(addr: SocketAddr, metrics: Arc<Metrics>) -> anyhow::Result<JoinHandle<()>> {
    ...
    info!(addr = %actual, "suwappu-l2-prover metrics: bound");

    let handle = tokio::spawn(async move {
        if let Err(err) = axum::serve(listener, app).await {
            warn!(error = %err, "suwappu-l2-prover metrics: server exited");
        }
    });
    Ok(handle)
}
