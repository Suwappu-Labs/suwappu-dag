...
//!   to the sequencer daemon (`suwappu_getUnprovenBatch` /
//!   `suwappu_submitIntent`), but that wiring lands in a follow-up.
//! - **How a batch is proven** — [`Prover`]. The real implementation
//!   runs SP1 over the `suwappu-l2-stm` guest and wraps the Plonky3 FRI
...
/// the real `suwappu_l2_sequencer::Batch` is richer, but threading it
...
pub trait UnprovenBatchSource: Send + Sync + 'static {
    ...
    /// `Intent::CommitL2StateRoot` via `suwappu_submitIntent`).
