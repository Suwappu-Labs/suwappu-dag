//! `suwappu-validator-program` binary.
//!
...
#[command(
    name = "suwappu-validator-program",
...
struct Args {
    ...
    /// start.
    #[arg(long, env = "SUWAPPU_PROGRAM_DATABASE_URL")]
    database_url: String,
    ...
    #[arg(
        long,
        default_value = "https://rpc.testnet.suwappu.globalsettlement.com",
        env = "SUWAPPU_PROGRAM_RPC_URL"
    )]
    rpc_url: String,

    /// HTTP bind address. Behind a Route53 A record for
    /// `program.testnet.suwappu.globalsettlement.com`.
    #[arg(long, default_value = "0.0.0.0:8090", env = "SUWAPPU_PROGRAM_BIND")]
    bind: SocketAddr,

    /// Bearer token gating the `/admin/*` endpoints. Foundation-
    /// supplied; rotate via Secrets Manager + systemd restart.
    #[arg(long, env = "SUWAPPU_PROGRAM_ADMIN_TOKEN")]
    admin_token: String,
}
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("suwappu_validator_program=info,axum=warn,tower_http=info,sqlx=warn")
        }))
        .init();
    ...
    info!(
        ...
        bind = %args.bind,
        "suwappu-validator-program: starting"
    );
    ...
    init_db(&pool).await?;
    info!("suwappu-validator-program: schema migrated");
    ...
    info!(addr = %actual, "suwappu-validator-program: HTTP server bound");
