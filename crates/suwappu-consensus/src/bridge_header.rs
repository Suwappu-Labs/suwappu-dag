...
//! **side-attestation** over a suwappu-dag block header. It is the Rust half of
...
use suwappu_crypto::mldsa::{sign, verify, PublicKey, SecretKey, Signature};
...
/// A single validator's ML-DSA side-attestation over a suwappu-dag block header.
///
...
/// destination `SuwappuDagQuorumHeaderOracle.submitHeader`. It carries the signing
...
/// suwappu-dag BLAKE3 L1 state root (`ExecutionReport::post_root`), which is **not**
...
pub struct HeaderAttestation {
    ...
    /// The suwappu-dag BLAKE3 L1 state root after executing the block at
