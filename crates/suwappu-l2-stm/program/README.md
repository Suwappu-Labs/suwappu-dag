# suwappu-l2-stm-program

SP1 guest program for the SUWAPPU L2 state-transition function.
...
the suwappu-dag workspace** so the host build doesn't need the
zkVM toolchain.

## What it does

The guest reads a `suwappu_l2_stm::BatchInput` from SP1's stdin,
...
The substrate-side verifier (`suwappu-l2-verifier-precompile`)
...
The resulting `../elf/suwappu-l2-stm-program` ELF is what the
...
```sh
# From the suwappu-dag repo root
cargo prove run --release --release-elf ../elf/suwappu-l2-stm-program
```

## Native ↔ guest equivalence

The shared `suwappu-l2-stm` lib (`../`) guarantees that the
...
```
        L2 user                          suwappu-l2-bridge (relayer)
            │                                       ▲
            ▼                                       │
   suwappu-l2-sequencer (Phase 2.2)                     │
            │  - mempool, batch build               │
        ...
        BatchInput                                  │
            ...
            ▼                                       │
  ┌────────────────────────┐                        │
  │  suwappu-l2-stm (host lib) │  ←──  shared lib       │
  └────────────────────────┘                        │
            │                                       │
            ▼                                       │
   suwappu-l2-stm-program (THIS)  →  ELF  →  prover daemon (Phase 2.1)
            │                              │
            ▼                              ▼
   240 B public inputs              Groth16 BN254 proof (260 B)
                                           │
                                           ▼
                          suwappu-l2-verifier-precompile (#97)
                                           │
                                           ▼
...
- `crates/suwappu-l2-verifier-precompile/src/lib.rs` — the on-chain
