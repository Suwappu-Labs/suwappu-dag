...
//! `PROPTEST_CASES=10000 cargo test --release -p suwappu-l2-stm`.
...
proptest! {
    ...
    /// `suwappu_l2_verifier_precompile::public_inputs::*` specifies.
    ...
    fn public_inputs_layout_round_trips(
        ...
        let output = suwappu_l2_stm::BatchOutput {
            new_l2_state_root: new_l2,
            da_commitment: da_commit,
            confidential_root: conf_root,
            withdrawals: vec![],
            withdrawals_root: suwappu_l2_stm::empty_withdrawals_root(),
            ledger: BTreeMap::new(),
        };
