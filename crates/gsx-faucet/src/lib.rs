//! `gsx-faucet` — devnet test-token faucet.
//!
...
//! `gsx_mempool::LeakyBucket` (same primitive as the F1 RPC limiter).
//!
//! Why not depend on `gsx-node`? The faucet only needs ~50 LoC worth
//! of intent encoding + signing. `gsx-node` pulls in the entire
//! validator daemon + gsxdb-bridge. Keeping the faucet's dep tree
...
//! `gsx_node::client::intent_signing_digest` produces — otherwise
...
use gsx_crypto::mldsa::{self, PublicKey, SecretKey};
...
pub struct DripAck {
    /// 32-byte intent hash, suitable for `gsx_getTransaction` lookup
    /// once the intent commits.
    #[serde(with = "hex_bytes")]
    pub tx_hash: [u8; 32],
    /// Amount actually dripped, in GSX. Always equals the faucet's
...
mod tests {
    ...
    #[test]
    fn digest_matches_gsx_node_recipe() {
        ...
        let digest = intent_signing_digest("gsx-devnet", &intent).unwrap();
