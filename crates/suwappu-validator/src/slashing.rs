...
//! validator that drops below `VALIDATOR_STAKE_THRESHOLD_SUWAPPU` is no
...
pub fn slash_validator(
    ...
    let current = registry.get(voter)?.stake_suwappu;
...
mod tests {
    use super::*;
    use crate::{ValidatorMember, VALIDATOR_STAKE_THRESHOLD_SUWAPPU};
    ...
    fn double_vote_slashes_thirty_percent() {
        ...
        let stake = VALIDATOR_STAKE_THRESHOLD_SUWAPPU * 10; // 250,000
        ...
        assert_eq!(r.get(0).unwrap().stake_suwappu, slash.remaining_stake);
    }
    ...
    fn slash_below_floor_keeps_member_seated() {
        // 25,000 SUWAPPU floor; after a 30% slash, remaining = 17,500 < floor.
