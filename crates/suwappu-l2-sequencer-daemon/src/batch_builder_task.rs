...
//!   ([`suwappu_l2_stm::execute_batch`]) gives us this, but
...
use blake3::Hasher;
use suwappu_l2_sequencer::{Batch, BatchBuilder, BuildContext, TxMempool, MAX_TXS_PER_BATCH};
...
impl BatchBuilderTaskConfig {
    ...
    /// `BLAKE3("suwappu-l2-chain-" || chain_id)`.
    pub fn derive_l2_chain_id_hash(chain_id: &str) -> [u8; 32] {
        let mut h = Hasher::new();
        h.update(b"suwappu-l2-chain-");
...
pub async fn run_one_tick<C: L1Client>(
    ...
    // `suwappu_l2_stm::execute_batch`. Stub for now.
    ...
    let persist_result = {
        ...
        // (`suwappu_execution::force_include::tx_hash`), so the
...
mod tests {
    ...
    fn l2_chain_id_hash_derivation_is_deterministic() {
        let a = BatchBuilderTaskConfig::derive_l2_chain_id_hash("suwappu-l2-testnet-1");
        let b = BatchBuilderTaskConfig::derive_l2_chain_id_hash("suwappu-l2-testnet-1");
        assert_eq!(a, b);
        let c = BatchBuilderTaskConfig::derive_l2_chain_id_hash("suwappu-l2-mainnet");
        assert_ne!(a, c);
    }
    ...
    // `blake3("suwappu-l2-chain-" || "test-chain")` and compare against
    ...
    /// digest. Solidity contract: `blake3("suwappu-l2-chain-" || chainId)`.
    ...
    fn l2_chain_id_hash_pinned_vector() {
        ...
        // compute blake3("suwappu-l2-chain-" || "test-chain") and
    ...
    fn da_commitment_matches_plain_blake3() {
        use suwappu_l2_sequencer::{BatchBuilder, BuildContext, PendingTx};
    ...
    fn batch_header_public_inputs_is_240_bytes() {
        use suwappu_l2_sequencer::BatchHeader;
