...
//! suwappu-validator-program shape).
...
/// ## Why TOML
///
/// Matches the suwappu-validator-program + suwappu-faucet + suwappu-node
...
pub struct SequencerConfig {
    ...
    /// per `suwappu_l2_stm::to_public_inputs`.
    ...
    /// key (PEM or hex per the suwappu-faucet convention). The
    ...
    /// `/var/lib/suwappu-l2-sequencer` to match the deploy layout.
    #[serde(default = "default_data_dir")]
    pub data_dir: String,
}

fn default_data_dir() -> String {
    "/var/lib/suwappu-l2-sequencer".to_string()
}
...
mod tests {
    ...
    fn minimal_toml() -> &'static str {
        r#"
l1_rpc_url = "https://rpc.testnet.suwappu.globalsettlement.com"
l2_chain_id = "suwappu-l2-testnet-1"
rpc_bind_addr = "0.0.0.0:8546"
signer_key_path = "/etc/suwappu/sequencer.key"
"#
    }
...
signer_key_path = "/k"
data_dir = "/mnt/state/suwappu-l2"
"#;
        let cfg: SequencerConfig = toml::from_str(with_dir).unwrap();
        cfg.validate().unwrap();
        assert_eq!(cfg.data_dir, "/mnt/state/suwappu-l2");
...
"#;
    ...
    fn load_from_missing_path_surfaces_io_error() {
        let err = SequencerConfig::load_from_path("/tmp/suwappu-l2-seq-config-does-not-exist.toml")
            .unwrap_err();
