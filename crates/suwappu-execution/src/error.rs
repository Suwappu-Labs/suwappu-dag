...
pub enum ExecutionError {
    ...
    /// type is the canonical `BalanceSlot::u128` of `suwappu-db`; we surface
    /// the overflow defensively even though it is unreachable under
    /// realistic SUWAPPU supply caps.
    #[error(
    ...
    /// `crates/suwappu-execution/src/reserved.rs`: L2 registry,
    ...
    /// Wraps `suwappu_l2_verifier_precompile::VerifyError`'s display
    ...
    /// Wraps `suwappu_l2_bridge::MerkleError`'s display for diagnostics.
    ...
    /// under realistic SUWAPPU supply caps, but surfaced defensively
