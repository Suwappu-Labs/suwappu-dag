...
use gsxdb_bridge::{Bridge, Intent as SuwappuIntent, RejectReason};
use gsxdb_state::{Address as SuwappuAddress, State};
...
impl Substrate for SuwappuDbSubstrate {
    ...
    fn apply_intent(&mut self, intent: &Intent) -> Result<(), ExecutionError> {
        ...
        match intent {
            ...
            // entry + enforces replay rejection. `SuwappuDbSubstrate`
            // cannot mirror the write yet — it depends on
            // suwappu-db v0.2.0's `bytes_state` surface.
            //
    ...
    fn state_root(&self) -> [u8; 32] {
        // suwappu-db computes the state root through gsxdb-state::StateTree
...
mod tests {
    ...
    /// `Intent::PostL2DAv2` must fail loud on `SuwappuDbSubstrate` — the
    /// suwappu-db backend can't yet write the DA-anchor registry
    /// (depends on `bytes_state` in suwappu-db v0.2.0), so accepting the
    ...
    /// contract.
    #[test]
    fn post_l2_da_v2_rejected_on_suwappu_db() {
        let mut s = SuwappuDbSubstrate::new();
        let err = s
