...
//! implementation that mirrors `suwappu-db`'s `BalanceStore` interface,
//! sufficient for the DAG-S10 exit gate. When the suwappu-db v0.1.0 tag is
//! cut on GitHub, the real wrapper will be `SuwappuDbSubstrate`, a thin
...
/// shape policy in suwappu-db's launch-readiness sprint.
...
/// SUWAPPU per the strategic plan Track G "Slashed-stake
...
pub enum Intent {
    ...
    MintInflation {
        ...
        epoch: u64,
        /// SUWAPPU credited to `authority_rewards_pool_address`.
        authority_share: Balance,
        /// SUWAPPU credited to `validator_rewards_pool_address`.
        validator_share: Balance,
        /// SUWAPPU credited to `treasury_address` (foundation
    ...
    SetL2VerifyingKey {
        /// L2 chain identifier hash — `BLAKE3("suwappu-l2-chain-"
    ...
    L1Lock {
        ...
        amount: Balance,
        /// Asset being bridged. `None` = native SUWAPPU (current
        ...
        /// be `AssetStatus::Active`. Native-SUWAPPU balance
    ...
    L2BurnProven {
        ...
        /// `L1Lock::asset_id`. `None` = native SUWAPPU,
    ...
    ///   bounty (10% of liveness-bond slash, capped 1M SUWAPPU),
    ...
    ///
    /// Track G "Sequencer bonding": 15M SUWAPPU safety bond,
    ...
    /// Deposit SUWAPPU into the Authority Ring stake pool,
    ...
    /// helper (`crates/suwappu-node/src/codec.rs`) silently ignores
    ...
    /// `crates/suwappu-l2-sequencer/src/lib.rs`, so off-chain auditors
    ...
    PostL2DAv2 {
        ...
        /// the same suwappu-dag substrate.
        l2_chain_id_hash: [u8; 32],
    },
}
...
pub trait Substrate {
    ...
    /// support bytes-state (suwappu-db v0.1.0) inherit the safe
    ...
    /// BLAKE3 over:
    ///   `"SUWAPPU-STATE-ROOT-V2"
    ///    || balances_root
    ///    || bytes_state_root`
    /// where
    ///   balances_root    = BLAKE3("SUWAPPU-BALANCES-V1"  || foreach (addr,bal) asc: addr(20) || bal_be(16))
    ///   bytes_state_root = BLAKE3("SUWAPPU-BYTES-STATE-V1" || foreach (addr,data) asc: addr(20) || data.len() as u32 BE || data)
    ///
...
/// concrete substrate (in-memory vs suwappu-db) can be selected at runtime.
...
impl InMemorySubstrate {
    ...
    /// Per-slot Authority Ring bonded capital, in SUWAPPU base units.
...
impl Substrate for InMemorySubstrate {
    ...
    fn apply_intent(&mut self, intent: &Intent) -> Result<(), ExecutionError> {
        match intent {
            ...
            // crates/suwappu-execution/src/l2_state.rs.
            Intent::CommitL2StateRoot {
                ...
                use suwappu_l2_verifier_precompile::public_inputs as pi;
            ...
            // should additionally validate via suwappu-l2-bridge
            ...
            Intent::L1Lock {
                ...
                // must exist + be Active. Native-SUWAPPU
            ...
            // merkle_path's byte-shape via suwappu-l2-bridge in
            // the meantime.
            Intent::L2BurnProven {
                batch_id,
                recipient,
            ...
            // suwappu-fastpath; downtime via the validator-program
            ...
            Intent::SlashSequencer {
                ...
                // 1M SUWAPPU) paid from treasury to the
            ...
            // "Sequencer bonding":
            //
            // > "Safety bond: 15,000,000 SUWAPPU. Matches Tier A
...
mod tests {
    ...
    fn intent_bincode_discriminants_are_pinned() {
        // The canonical hash-path codec (mirrors suwappu-mempool and
        // suwappu-node's codec): bincode 1.x-compatible legacy config.
    ...
    /// suwappu-dag substrate without batch-id collisions.
    #[test]
    ...
    /// produces a >1M-SUWAPPU 10% bounty but the submitter only
    ...
    /// Native-SUWAPPU balance accounting still applies (phase 1
    ...
    fn depth_one_tree(leaf_hash: [u8; 32], sibling: [u8; 32]) -> ([u8; 32], Vec<u8>, Vec<u8>) {
        let root = suwappu_l2_bridge::hash_inner_node(&leaf_hash, &sibling);
    ...
    fn l2_burn_proven_merkle_forged_path_rejects() {
        ...
        let recipient = addr(3);
        let real_leaf = suwappu_l2_bridge::BurnLeaf {
            l2_chain_id_hash: &[0u8; 32],
            batch_id: 7,
            recipient: &recipient,
            amount: 100,
    ...
    fn admit_authority_inserts_active_record() {
        ...
        assert_eq!(rec.stake_suwappu, 15_000_000);
    ...
    fn admit_validator_duplicate_slot_rejected() {
        ...
        assert_eq!(s.validator_record(42).unwrap().stake_suwappu, 3_000_000);
    }
    ...
    fn multiple_validators_independent() {
        ...
        for i in 0..10 {
            s.apply_intent(&Intent::AdmitValidator {
                validator_id: i,
                stake_suwappu: 3_000_000 + i as u64,
                mldsa_public_key: vec![i as u8; 1952],
                bls_public_key: vec![i as u8; 48],
            })
            .unwrap();
    ...
    fn distribute_rewards_recipient_overflow_rolls_back_atomically() {
        ...
        // Park addr(2) at MAX so a 1-SUWAPPU credit overflows.
        s.apply_intent(&Intent::GenesisAllocation {
