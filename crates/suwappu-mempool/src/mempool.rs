...
/// Same value the JSON-RPC `suwappu_submitIntent` returns and the
...
impl Mempool {
    ...
    pub fn submit(
        ...
        //    bincode config here MUST match the rest of suwappu-node's
        //    intent serialization (`suwappu_node::codec::encode`) so the
