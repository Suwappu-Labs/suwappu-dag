...
//!    `VALIDATOR_STAKE_THRESHOLD_GSX` (25,000 GSX).
...
//! `gsx-consensus::validator_quorum_threshold` and the two are verified
...
use crate::{Stake, ValidatorId, VALIDATOR_RING_MAX, VALIDATOR_STAKE_THRESHOLD_GSX};
...
impl ValidatorRegistry {
    ...
    pub fn admit(&mut self, member: ValidatorMember) -> Result<(), AdmissionError> {
        if member.stake_gsx < VALIDATOR_STAKE_THRESHOLD_GSX {
            return Err(AdmissionError::StakeBelowFloor {
                posted: member.stake_gsx,
                floor: VALIDATOR_STAKE_THRESHOLD_GSX,
            });
        }
    ...
    pub fn total_stake(&self) -> Stake {
        self.members.values().map(|m| m.stake_gsx).sum()
    }
...
mod tests {
    ...
    fn admit_below_floor_fails() {
        let mut r = ValidatorRegistry::new();
        let err = r.admit(member(0, VALIDATOR_STAKE_THRESHOLD_GSX - 1));
    ...
    fn admit_duplicate_fails() {
        ...
        r.admit(member(0, VALIDATOR_STAKE_THRESHOLD_GSX)).unwrap();
        assert_eq!(
            r.admit(member(0, VALIDATOR_STAKE_THRESHOLD_GSX)),
    ...
    fn admit_full_ring_fails() {
        ...
        for i in 0..VALIDATOR_RING_MAX as u32 {
            r.admit(member(i, VALIDATOR_STAKE_THRESHOLD_GSX)).unwrap();
        }
    ...
    fn total_stake_sums_members() {
        ...
        assert_eq!(r.total_stake(), 5 * VALIDATOR_STAKE_THRESHOLD_GSX);
    }

    #[test]
    fn quorum_threshold_strictly_above_two_thirds() {
        let mut r = ValidatorRegistry::new();
        // 9 validators each at the floor (25,000 GSX). Total stake
