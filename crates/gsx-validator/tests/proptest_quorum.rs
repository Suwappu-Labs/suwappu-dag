...
//! (`gsx_consensus::quorum_threshold` for the count side,
//! `gsx_consensus::validator_quorum_threshold` for the stake side).
...
use gsx_authority::{
    AuthorityMember, AuthorityRegistry, AUTHORITY_RING_MAX, AUTHORITY_STAKE_THRESHOLD_GSX,
};
use gsx_consensus::{quorum_threshold as consensus_authority_threshold, StakeTable};
use gsx_validator::{
    Stake, ValidatorMember, ValidatorRegistry, VALIDATOR_RING_MAX, VALIDATOR_STAKE_THRESHOLD_GSX,
};
...
fn build_authority(n: u32) -> AuthorityRegistry {
    ...
    for i in 0..n {
        r.admit(auth(i, AUTHORITY_STAKE_THRESHOLD_GSX)).unwrap();
    }
    r
}
...
proptest! {
    ...
    fn quorum_math_matches_paper(
        ...
        per_validator_stake in VALIDATOR_STAKE_THRESHOLD_GSX..=(VALIDATOR_STAKE_THRESHOLD_GSX * 4),
    ) {
        // Authority side.
        ...
        let stake_table = StakeTable::from_entries(
            val_reg.members().map(|m| (m.id, m.stake_gsx)),
        );
        let consensus_threshold = gsx_consensus::validator_quorum_threshold(&stake_table);
    ...
    fn authority_admission_enforces_invariants(
        ...
        stake in 0u64..=(AUTHORITY_STAKE_THRESHOLD_GSX * 2),
        ...
        match r.admit(auth(candidate_id, stake)) {
            Ok(()) => {
                prop_assert!(stake >= AUTHORITY_STAKE_THRESHOLD_GSX);
                prop_assert!(r.len() <= AUTHORITY_RING_MAX);
                prop_assert!(r.contains(candidate_id));
            }
            Err(gsx_authority::AdmissionError::StakeBelowFloor { posted, floor }) => {
                prop_assert!(posted < floor);
                prop_assert_eq!(floor, AUTHORITY_STAKE_THRESHOLD_GSX);
            }
            Err(gsx_authority::AdmissionError::RingFull { size, max }) => {
                prop_assert_eq!(size, AUTHORITY_RING_MAX);
                prop_assert_eq!(max, AUTHORITY_RING_MAX);
            }
            Err(gsx_authority::AdmissionError::DuplicateMember(id)) => {
    ...
    fn validator_admission_enforces_invariants(
        stake in 0u128..=(VALIDATOR_STAKE_THRESHOLD_GSX * 4),
        n_seed in 0u32..=64,
    ) {
        let mut r = build_validator(n_seed, VALIDATOR_STAKE_THRESHOLD_GSX);
        let candidate_id = n_seed;

        match r.admit(vmem(candidate_id, stake)) {
            Ok(()) => {
                prop_assert!(stake >= VALIDATOR_STAKE_THRESHOLD_GSX);
                prop_assert!(r.len() <= VALIDATOR_RING_MAX);
            }
            Err(gsx_validator::AdmissionError::StakeBelowFloor { posted, floor }) => {
                prop_assert!(posted < floor);
                prop_assert_eq!(floor, VALIDATOR_STAKE_THRESHOLD_GSX);
            }
            Err(gsx_validator::AdmissionError::RingFull { .. }) => {
                // Won't fire in this property because n_seed is bounded
                // below VALIDATOR_RING_MAX; we still accept the error.
            }
            Err(gsx_validator::AdmissionError::DuplicateMember(id)) => {
                prop_assert_eq!(id, candidate_id);
            }
        }
    }
    ...
    fn removal_decreases_membership(
        n in 1u32..=32,
        stake_per in VALIDATOR_STAKE_THRESHOLD_GSX..=(VALIDATOR_STAKE_THRESHOLD_GSX * 4),
        ...
        prop_assert_eq!(removed.stake_gsx, stake_per);
