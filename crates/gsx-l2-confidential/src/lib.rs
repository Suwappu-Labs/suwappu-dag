//! gsx-l2-confidential — host-side PQ-confidential transfer
//! primitives for the L2 (Track H, issue #156).
//!
//! Implements the construction ratified in
//! `gsx-strategy/docs/mainnet-plan.md` Track H: an engineering
...
//!   on SHA3-256 + the existing `gsx-crypto::hash` domain-tag
//!   helpers from PR #170:
...
//!     from `SHA3-256-domain(GSX_L2_VIEWING_KEY_V1, sk_seed)`)
...
//!
//! - **Commitment**: `cm = SHA3-256-domain(GSX_L2_NOTE_COMMIT_V1,
...
//! - **Nullifier**: `nf = SHA3-256-domain(GSX_L2_NULLIFIER_V1,
//!   nk ‖ cm ‖ position_le)` — mechanical SHA3-swap of
//!   Zcash Sapling's PRF^nf pattern.
//! - **Nullifier key**: `nk = SHA3-256-domain(GSX_L2_NF_KEY_V1,
//!   sk_seed)` — derives a per-account nullifier key from the
//!   master seed.
//! - **L2 address**: `addr = SHA3-256-domain(GSX_L2_ADDRESS_V1,
...
/// `cm = SHA3-256-domain(GSX_L2_NOTE_COMMIT_V1, v_le(8) ‖ r(32) ‖ pk_owner)`.
///
...
pub fn commit_note(note: &Note) -> Result<Commitment, ConfidentialError> {
    ...
    let digest = sha3_256_domain(GSX_L2_NOTE_COMMIT_V1, &buf);
    Ok(Commitment(digest))
}

/// Derive the per-account nullifier key
/// `nk = SHA3-256-domain(GSX_L2_NF_KEY_V1, sk_seed)`.
///
...
pub fn derive_nullifier_key(sk_seed: &[u8; NF_KEY_SEED_BYTES]) -> NullifierKey {
    let digest = sha3_256_domain(GSX_L2_NF_KEY_V1, sk_seed);
    NullifierKey(digest)
}

/// Compute the nullifier for a spent note
/// `nf = SHA3-256-domain(GSX_L2_NULLIFIER_V1, nk ‖ cm ‖ position_le(8))`.
///
...
pub fn compute_nullifier(nk: &NullifierKey, cm: &Commitment, position: u64) -> Nullifier {
    ...
    let digest = sha3_256_domain(GSX_L2_NULLIFIER_V1, &buf);
    Nullifier(digest)
}

/// Derive the 20-byte L2 address from an ML-DSA-65 public key
/// via `addr = SHA3-256-domain(GSX_L2_ADDRESS_V1, pk)[..20]`.
///
...
/// existing reserved-address gate from `gsx-execution::reserved`
...
pub fn derive_l2_address(pk: &[u8]) -> Result<[u8; L2_ADDRESS_BYTES], ConfidentialError> {
    ...
    let digest = sha3_256_domain(GSX_L2_ADDRESS_V1, pk);
...
mod tests {
    ...
    fn domains_separate_under_identical_input() {
        let buf = vec![0u8; 32];
        let h_note = sha3_256_domain(GSX_L2_NOTE_COMMIT_V1, &buf);
        let h_nf = sha3_256_domain(GSX_L2_NULLIFIER_V1, &buf);
        let h_nk = sha3_256_domain(GSX_L2_NF_KEY_V1, &buf);
        let h_addr = sha3_256_domain(GSX_L2_ADDRESS_V1, &buf);
