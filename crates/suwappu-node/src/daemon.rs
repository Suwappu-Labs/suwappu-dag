...
pub(crate) struct StateInner {
    ...
    /// substrate (in-memory today; suwappu-db once its seeding/credit
    ...
    fn new(manifest: &GenesisManifest) -> Self {
        ...
        let prebalances = manifest
            ...
            .filter_map(|p| {
                ...
                {
                    Some(addr) => Some((addr, p.balance_suwappu as u128)),
                    None => {
    ...
    pub async fn start(cfg: NodeConfig, manifest: GenesisManifest) -> anyhow::Result<Self> {
        ...
        let self_mldsa_sk = {
            ...
            if path.as_os_str() == "/dev/null" {
                ...
                let sk = suwappu_crypto::mldsa::SecretKey::from_bytes(&key_bytes)
        ...
        if let Some(rpc_addr) = cfg.rpc_listen {
            tracing::info!(addr = %rpc_addr, "suwappu-rpc server starting");
...
async fn run_inbox(
    ...
    self_mldsa_sk: Arc<suwappu_crypto::mldsa::SecretKey>,
    network_id: String,
) {
...
mod tests {
    ...
    fn test_key_files(
        ...
        Vec<suwappu_crypto::mldsa::SecretKey>,
        Vec<suwappu_crypto::mldsa::PublicKey>,
    ) {
        ...
        for i in 0..n {
            ...
            let path = std::env::temp_dir().join(format!("suwappu-test-sk-{}-{}.bin", i, pid));
    ...
    /// with `for i in $(seq 1 20); do cargo test -p suwappu-node \
