//! Adapter that implements `suwappu_rpc::StateView` against the running
//! daemon's `State`. This module is the only bridge between the daemon
//! and the JSON-RPC server — `suwappu-rpc` itself has no dependency on
//! `suwappu-node`, so the trait-and-adapter pattern keeps the dep graph
//! a DAG.
//!
...
fn intent_to_view(intent: &Intent) -> IntentView {
    match intent {
        ...
        Intent::AdmitAuthority {
            ...
            stake_suwappu: stake_suwappu.to_string(),
...
/// surface (`suwappu_submitIntent` + read methods). DAG-S31.4 / A3: the
...
/// suwappu-node `Event` stream into the JSON-safe `EventView` shape the
...
impl StateView for NodeStateView {
    ...
    async fn validator_snapshot(&self) -> Vec<ValidatorMemberView> {
        ...
        reg.members()
            .map(|m| ValidatorMemberView {
                id: m.id,
                stake_suwappu: m.stake_suwappu.to_string(),
            })
            .collect()
    }
    ...
    async fn stake_for(&self, authority_id: u32) -> Option<u128> {
        let reg = self.state.authority_registry.read().await;
        reg.get(authority_id).map(|m| m.stake_suwappu as u128)
    }
    ...
    async fn l2_state_root(&self, l2_chain_id_hash: [u8; 32]) -> [u8; 32] {
        ...
        use suwappu_execution::l2_state::L2BatchKey;
        registry
            .state_roots
            .range(
                L2BatchKey {
    ...
    async fn force_include_registry_bytes(&self) -> Vec<u8> {
        ...
        inner
            .substrate
            .read_bytes(&suwappu_execution::reserved::force_include_registry_address())
            .unwrap_or_default()
    }
