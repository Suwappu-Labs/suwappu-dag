//! suwappu-metrics — multi-mode event-log analyzer for bank-compliance
...
//! suwappu-metrics --logs us-east-1=us.ndjson --logs eu-west-1=eu.ndjson > certs.csv
//!
//! # Bank-compliance e2e join:
//! suwappu-metrics --mode e2e --loadgen-csv loadgen.csv \
...
#[derive(Parser, Debug)]
#[command(
    name = "suwappu-metrics",
    version,
    about = "Join SUWAPPU validator event logs into CSV"
)]
struct Args {
    ...
    /// Optional loadgen CSV from suwappu-loadgen stdout. Required for
...
fn emit_e2e_mode(
    ...
    if dropped_out_of_window > 0 {
        eprintln!(
            "suwappu-metrics e2e: filtered {} committed events outside window [{}, {}]",
            dropped_out_of_window, lo, hi
        );
    }

    // DAG-S29.1: drop rows where the matched committed_ms is BEFORE
    // submitted_ms. Cause: suwappu-loadgen seeds its RNG deterministically
    ...
    if dropped_stale > 0 {
        eprintln!(
            "suwappu-metrics e2e: warning — dropped {} (tx_hash, region) pairs where \
             committed_ms < submitted_ms (stale hash match from a prior \
             campaign run with the same deterministic loadgen seed). \
             Pass `--seed <unique>` to suwappu-loadgen to suppress.",
            dropped_stale
        );
    }
    Ok(())
...
fn emit_tps_mode(events: &[Event], bucket_ms: u64) {
    ...
    if commits_total > 0 && commits_with_intents == 0 {
        eprintln!(
            "suwappu-metrics tps: warning — {} committed events lacked `intent_hashes`; \
        ...
        eprintln!(
            "suwappu-metrics tps: {} of {} committed events had no `intent_hashes` \
