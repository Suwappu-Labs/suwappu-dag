# gsx-mldsa-precompile

On-chain **ML-DSA-65 (FIPS 204)** signature-verification precompile core for the
GSX bridge — the load-bearing piece of the on-chain post-quantum verification
...
that `gsx-crypto` already uses for validator/consensus signatures, so it is
...
`gsx-lattice-protocol/docs/security/audits/suwappu/P5b_ONCHAIN_PQ.md`.)

## Status (2026-06-07)

- ✅ **Core verifier + I/O encoding: implemented and LOCALLY VERIFIED.**
  `cargo test -p gsx-mldsa-precompile` → 8/8 green against real ML-DSA-65 keys:
...
- ⏭️ **EVM registration glue (CI-verified next step).** Register as a Monad
  precompile in `gsx-revm` at a fixed address (e.g. `0x0101`), following the
  `Precompile::new(PrecompileId::custom("MLDSA65_VERIFY"), addr, run_fn)` pattern
  in `gsx-revm/crates/gsx-revm/src/precompiles.rs`. The `run_fn` is a thin
  adapter: gas check → `gsx_mldsa_precompile::verify(input)` → `PrecompileOutput`.
  Gas: set ~8–12k (cf. EIP-8051's 4,500 for ML-DSA-44). This step needs a cold
  `gsx-revm` (revm v34) build, run via CI — not on the local Mac (workspace-build
  rule in GSXHELPER.md).
- ⏭️ **End-to-end wiring blocked on EVM integration.** gsx-dag main does not yet
...
criticals in gsx-lattice-protocol): a relayer can no longer assert a commit the
