//! gsx-l2-prover CLI entry point (Track G G4.1, #104).
//!
...
#[command(
    name = "gsx-l2-prover",
    version,
    about = "GSX L2 prover daemon (Track G G4.1, #104)"
)]
struct Cli {
...
fn main() -> Result<()> {
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new("info,gsx_l2_prover=debug"));
...
async fn run(cfg: ProverConfig) -> Result<()> {
    ...
    info!(
        ...
        "gsx-l2-prover: starting"
    );
    warn!(
        "gsx-l2-prover: SP1 proving is GATED (StubProver) — no proofs will be produced until \
         #88 (revm-in-guest) and #94 (sp1-sdk decision) land. See gsx_l2_prover::prover."
    );
    ...
    // (`gsx_getUnprovenBatch` / `gsx_submitIntent`); that wiring lands
    ...
    info!("gsx-l2-prover: source = mock (real sequencer JSON-RPC client lands in a follow-up)");
    ...
    info!("gsx-l2-prover: ctrl-c received, shutting down");
