...
struct MockState {
    ...
    event_tx: tokio::sync::broadcast::Sender<suwappu_rpc::context::EventView>,
}
...
async fn get_authority_registry_returns_ordered_list() {
    ...
    let resp = post_rpc(
        ...
        json!({
            "jsonrpc": "2.0",
            "id": "two",
            "method": "suwappu_getAuthorityRegistry",
        }),
    )
    .await;
...
async fn get_validator_registry_returns_stake_as_string() {
    ...
    let resp = post_rpc(
        ...
        json!({
            "jsonrpc": "2.0",
            "id": 3,
            "method": "suwappu_getValidatorRegistry",
        }),
    )
    .await;
    ...
    assert_eq!(arr[0]["stake_suwappu"], "30000");
    assert!(arr[0]["stake_suwappu"].is_string());
}
...
async fn get_stake_object_params() {
    ...
    assert_eq!(resp["result"]["stake_suwappu"], "30000");
}
...
async fn get_block_not_found() {
    ...
    // -32000 application-level NotFound (consistent with suwappu_getStake).
    assert_eq!(resp["error"]["code"], -32000);
}
...
async fn get_force_include_registry_returns_hex() {
    ...
    let resp = post_rpc(
        ...
        json!({
            "jsonrpc": "2.0",
            "id": 204,
            "method": "suwappu_getForceIncludeRegistry",
        }),
    )
    .await;
...
async fn subscribe_events_returns_receiver_per_subscriber() {
    ...
    let view = suwappu_rpc::context::EventView {
        t_ms: 123,
        region: "v0".into(),
        lane: "main".into(),
    ...
    let (tx, _) = tokio::sync::broadcast::channel::<suwappu_rpc::context::EventView>(8);
