//! JSON-RPC 2.0 query API for the suwappu-dag node.
//!
//! Two transports:
//!
//! - `POST /` — JSON-RPC request/response for all unary methods
//!   (read + write).
//! - `GET /ws` — WebSocket subscription for `suwappu_subscribeEvents`
...
//! Read:
//! - `suwappu_getEpoch`
//! - `suwappu_getAuthorityRegistry`
//! - `suwappu_getValidatorRegistry`
//! - `suwappu_getStake { authority_id }`
//! - `suwappu_getBalance { address: hex }`
//! - `suwappu_getBlock { round }`
//! - `suwappu_getTransaction { tx_hash: hex }`
//! - `suwappu_getL1StateRoot`
//! - `suwappu_getL2StateRoot { l2_chain_id_hash: hex }`
//! - `suwappu_getForceIncludeRegistry`
//!
//! Write:
//! - `suwappu_submitIntent { intent: hex, signature: hex, signer_pubkey_hash: hex }`
//!
//! Streaming (WebSocket):
//! - `suwappu_subscribeEvents` — live stream of every emitted event
...
/// `suwappu-node` is wrapped in `RpcContext::from_state` in the daemon's
/// bind site.
...
pub async fn start_with_limits<S: StateView>(
    ...
    info!(addr = %actual, "suwappu-rpc server bound");
    ...
    let handle = tokio::spawn(async move {
        if let Err(err) = axum::serve(listener, make_service).await {
            tracing::error!(error = %err, "suwappu-rpc server exited");
        }
    });
    Ok(handle)
}
