...
//! - `GET /ws` — WebSocket subscription stream (`suwappu_subscribeEvents`).
//!
...
/// is generous for any read method; `suwappu_submitIntent` is bounded by
/// the ML-DSA verify step (~2 ms) and the mempool enqueue (~us).
pub const DEFAULT_REQUEST_TIMEOUT_MS: u64 = 30_000;

/// Hardening limits applied to the suwappu-rpc HTTP router. The defaults
...
async fn dispatch<S: StateView>(
    ...
    match method {
        "suwappu_getEpoch" => methods::get_epoch(state, params).await,
        "suwappu_getAuthorityRegistry" => methods::get_authority_registry(state, params).await,
        "suwappu_getValidatorRegistry" => methods::get_validator_registry(state, params).await,
        "suwappu_getStake" => methods::get_stake(state, params).await,
        ...
        "suwappu_getL1StateRoot" => methods::get_l1_state_root(state, params).await,
        "suwappu_getL2StateRoot" => methods::get_l2_state_root(state, params).await,
        "suwappu_getForceIncludeRegistry" => methods::get_force_include_registry(state, params).await,
