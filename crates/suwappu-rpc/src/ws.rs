//! WebSocket handler for `suwappu_subscribeEvents` (T6).
//!
...
//! `suwappu_getBlock` / `suwappu_getTransaction` for the last known
//! checkpoint, then subscribe.
//!
//! ## Backpressure policy
//!
//! The underlying `broadcast` channel has a fixed ring buffer (1024
//! slots on the daemon side — see `crates/suwappu-node/src/events.rs`).
...
//! reconnect and refill via `suwappu_getBlock` from their checkpoint.
//!
