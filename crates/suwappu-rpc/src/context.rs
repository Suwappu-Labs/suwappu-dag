//! Read-only view of the suwappu-dag node state that the RPC methods consume.
//!
//! Defined as a trait so `suwappu-rpc` doesn't need a runtime dependency on
//! `suwappu-node` — the node implements `StateView` for its concrete `State`
...
pub struct AuthorityMemberView {
    ...
    pub id: u32,
    /// Posted stake in SUWAPPU (u64 fits — Authority stakes are bounded
...
/// (Validator stakes use u128 — see `suwappu_consensus::Stake`).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
...
/// `ValidatorMemberView::stake_suwappu`. A zero balance is a valid
...
pub struct BlockView {
    ...
    /// `suwappu_getBlock` response is self-sufficient — no follow-up
    /// `suwappu_getTransaction` calls needed to enumerate a block's txs.
    #[serde(default)]
    pub tx_hashes: Vec<String>,
}

/// JSON-safe projection of a single emitted event (T6). The shape
/// mirrors `suwappu_node::events::Event` field-for-field, kept in
/// `suwappu-rpc` so the trait doesn't drag in a dependency on `suwappu-node`.
...
///
/// A relayer polls each validator's `suwappu_getHeaderAttestation`, collects a set
/// whose stake clears the on-chain >2/3 threshold, sorts by
/// `keccak256(pubkey)`, and submits to `SuwappuDagQuorumHeaderOracle.submitHeader`.
...
/// `state_root` is the suwappu-dag BLAKE3 L1 root, which is not storage-provable;
...
pub trait StateView: Send + Sync + 'static {
    ...
    /// events. Indexers should catch up by querying `suwappu_getBlock`
...
pub enum SubmitIntentError {
    /// Intent bytes failed bincode decoding into `suwappu_execution::Intent`.
