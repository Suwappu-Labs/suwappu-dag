...
//!
//! - `gsx_last_committed_round` (gauge) — `inner.blocks_by_round`
//!   max key. Halt alarm's primary signal.
//! - `gsx_committed_rounds_total` (counter) — `state.committed.len()`.
//! - `gsx_mempool_size` (gauge) — `state.mempool.stats().queued`.
//! - `gsx_node_info{authority_id, region}` (gauge, always 1) — labels
//!   only; identifies this validator.
//! - `gsx_process_uptime_seconds` (gauge) — seconds since process
...
/// Static labels rendered on `gsx_node_info` so dashboards can
...
pub struct MetricsCounters {
    ...
    /// produce the `gsx_process_uptime_seconds` gauge.
    started_at: Instant,
...
pub(crate) async fn start_if_configured(
    ...
    let Some(addr) = addr else {
        info!("gsx-metrics: metrics_listen not set; endpoint disabled");
        return Ok(None);
    };
    ...
    info!(addr = %actual, "gsx-metrics: bound");

    let handle = tokio::spawn(async move {
        if let Err(err) = axum::serve(listener, app).await {
            warn!(error = %err, "gsx-metrics: server exited");
        }
    });
    Ok(Some(handle))
}
...
async fn render_health() -> impl IntoResponse {
    ...
    // (gsx_last_committed_round flat → page).
    "ok"
}
...
async fn render_metrics(State(state): State<MetricsState>) -> impl IntoResponse {
    ...
    let _ = writeln!(
        out,
        "gsx_node_info{{region=\"{}\",authority_id=\"{}\"}} 1",
        escape_label(&state.identity.region),
        state.identity.authority_id
    );

    let _ = writeln!(out, "# HELP gsx_last_committed_round Highest committed round seen by this validator. Halt-alarm signal: flat for >5 min → cluster has stopped progressing.");
    let _ = writeln!(out, "# TYPE gsx_last_committed_round gauge");
    let _ = writeln!(out, "gsx_last_committed_round {last_committed_round}");

    let _ = writeln!(out, "# HELP gsx_committed_rounds_total Number of distinct cert hashes this validator has marked committed.");
    ...
    let _ = writeln!(out, "# HELP gsx_metrics_scrapes_total Cumulative count of /metrics scrapes this process has served.");
    let _ = writeln!(out, "# TYPE gsx_metrics_scrapes_total counter");
    let _ = writeln!(out, "gsx_metrics_scrapes_total {scrapes_total}");
