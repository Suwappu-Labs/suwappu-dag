//! gsx-l2-sequencer — L2 batch sequencer (Track G G4.2, #105).
//!
//! ## Phase split
//!
...
//!     `gsx-l2-verifier-precompile::public_inputs` offsets)
...
use blake3::Hasher;
use gsx_l2_verifier_precompile::{public_inputs, L2_PUBLIC_INPUTS_BYTES};
...
/// input layout (per
/// `gsx-l2-verifier-precompile::public_inputs`); the
...
impl BatchHeader {
    ...
    /// per the offsets in `gsx_l2_verifier_precompile::public_inputs`.
...
pub struct BuildContext {
    ...
    pub prev_l1_state_root: [u8; 32],
    /// Multi-L2 namespacing — `BLAKE3("gsx-l2-chain-" || chain_id)`.
    pub l2_chain_id_hash: [u8; 32],
...
mod tests {
    ...
    /// public_inputs blob passes the gsx-l2-verifier-precompile
    /// format gates.
    #[test]
