...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "suwappu_indexer=info,tower_http=warn".into()),
        )
        .init();

    let cfg = IndexerConfig::parse();
    info!(?cfg, "suwappu-indexer: starting");
    ...
    if let Some(database_url) = &cfg.database_url {
        ...
        {
            info!("suwappu-indexer: connecting to Postgres + applying migrations",);
            let store = suwappu_indexer::PostgresStore::connect(database_url).await?;
            let latest = store.latest_round().await;
            match latest {
                Some(r) => info!(
                    latest_round = r,
                    "suwappu-indexer: resuming from persisted state — WS tail \
                     starts at live; commits between (latest_round+1, head] \
                     are missed until backfill via suwappu_getBlock lands"
                ),
                None => info!(
                    "suwappu-indexer: persistent store is empty; starting from \
                     live WS tail"
                ),
            }
        ...
        #[cfg(not(feature = "postgres"))]
        {
            ...
            anyhow::bail!(
                "--database-url was set but this binary was built without \
                 ...
                 `cargo build -p suwappu-indexer --features postgres`, or \
                 clear --database-url to fall back to in-memory storage.",
            );
        }
    } else {
        info!(
            "suwappu-indexer: using in-memory store (state lost on restart). \
    ...
    S: Store + Clone + 'static,
{
    ...
    if let Some(rpc_url) = cfg.rpc_url.clone() {
        info!(rpc_url = %rpc_url, "suwappu-indexer: running startup backfill");
        if let Err(e) = suwappu_indexer::backfill::catch_up(
            &*store,
            &rpc_url,
            suwappu_indexer::backfill::DEFAULT_MAX_PER_ITER,
        )
        .await
        {
            warn!(error = %e, "suwappu-indexer: backfill failed — continuing with live tail only");
        }
    } else {
        info!(
            "suwappu-indexer: --rpc-url not set; skipping startup backfill. \
    ...
    info!(addr = %actual, "suwappu-indexer: HTTP API bound");
    if let Err(e) = axum::serve(listener, app).await {
        warn!(error = %e, "suwappu-indexer: HTTP server exited");
    }
    Ok(())
}
