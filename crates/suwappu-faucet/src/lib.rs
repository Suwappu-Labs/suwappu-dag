...
/// via bincode legacy and delegates to [`suwappu_execution::intent_signing_digest`].
pub fn intent_signing_digest(network_id: &str, intent: &Intent) -> Result<[u8; 32], FaucetError> {
    ...
    Ok(suwappu_execution::intent_signing_digest(
        network_id,
        &intent_bytes,
    ))
}
...
/// or set `SUWAPPU_FAUCET_ADDRESS` on the binary — main.rs accepts an
/// explicit override.
...
impl Faucet {
    ...
    pub async fn drip(&self, peer_ip: IpAddr, address: [u8; 20]) -> Result<DripAck, FaucetError> {
        ...
        let intent_bytes = bincode::serde::encode_to_vec(&intent, bincode::config::legacy())
            .map_err(|e| FaucetError::Encode(e.to_string()))?;
        let digest = suwappu_execution::intent_signing_digest(&self.network_id, &intent_bytes);
