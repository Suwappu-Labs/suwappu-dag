...
pub const BURN_LEAF_DOMAIN_TAG: &[u8] = b"gsx-l2-burn-leaf-v1";

/// Domain tag for `L2BurnProven` merkle inner nodes. Length-
/// distinguished from the leaf tag. See IQ-008.
pub const BURN_NODE_DOMAIN_TAG: &[u8] = b"gsx-l2-burn-node-v1";
...
pub struct BurnLeaf<'a> {
    ...
    pub amount: Balance,
    /// Asset selector. `None` for native GSX; `Some` for a
    /// registered bridge asset.
    pub asset_id: Option<&'a [u8; 32]>,
}

impl<'a> BurnLeaf<'a> {
    /// Compute the canonical leaf hash per IQ-008's `gsx-l2-burn-leaf-v1`
...
/// `gsx-l2-burn-node-v1` scheme. Domain tag is length-distinguished
/// from the leaf tag.
#[must_use]
